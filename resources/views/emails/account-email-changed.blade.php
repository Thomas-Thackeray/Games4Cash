...
<head>
    ...
    <style>
        ...
        .alert-box { background: #fff5f5; border: 1px solid #fcc; border-radius: 8px; padding: 16px 20px; margin-bottom: 28px; }
        .alert-box p { margin: 0; font-size: 0.9rem; color: #c0392b; line-height: 1.6; }
...
<div class="wrapper">
    <div class="card">
        ...
        <div class="body">
            <h1 class="greeting">Hi {{ $user->first_name }},</h1>
            <p class="intro">We're letting you know that the email address on your {{ config('app.name') }} account was recently changed.</p>

            <div class="details-box">
                <p><strong>Account Changes</strong></p>
                <p>Previous email: <strong>{{ $oldEmail }}</strong></p>
                <p>New email: <strong>{{ $user->email }}</strong></p>
                <p>Changed at: <strong>{{ now()->format('d M Y, H:i') }} UTC</strong></p>
            </div>

            <div class="alert-box">
                <p><strong>Wasn't you?</strong> If you did not make this change, please contact us immediately so we can secure your account.</p>
            </div>

            <div class="cta-wrap">
                <a href="{{ url('/contact') }}" class="cta-btn">Contact Support</a>
            </div>

            <hr class="divider">

            <p class="body-note">This notification was sent to your previous email address as a security measure. If you made this change yourself, no further action is needed.</p>
        </div>
